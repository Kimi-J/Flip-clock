...
一款基于 Tauri 2 + React + TypeScript 构建的 Windows 桌面翻页时钟应用，支持全屏显示、系统屏幕保护程序、9 套精致主题与 3 种背景效果。

## 功能特性

- **全屏翻页时钟** — 启动即全屏，整块屏幕作为渲染区域，覆盖任务栏；数字采用单叶片 180° 连续翻页动画
- **系统屏幕保护程序** — 可注册为 Windows 系统屏保，空闲后自动启动；支持自定义等待时间（1-60 分钟），与系统控制面板设置同步
- **多显示器屏保** — 屏保模式下为每台显示器创建独立窗口，全部铺满；基于物理坐标定位，混合 DPI 下也能精准落位
- **9 套主题配色** — 极简黑 / 极简白 / 暖琥珀 / 晨雾白 / 午夜蓝 / 矩阵绿 / 像素界 / 霓虹波 / 水墨韵，每套主题拥有独立的视觉语言
- **3 种背景效果** — 极简（纯色）/ 极光（流动光晕）/ 星空（粒子星点）
- **12/24 小时制** — 一键切换显示格式
- **秒数 / 日期信息栏** — 可选开启/关闭秒数翻页与底部日期信息栏
- **窗口控制** — 鼠标移至右上角显示最小化/设置/关闭按钮
- **设置持久化** — 所有偏好通过 localStorage 保存，重启后自动恢复
...
性能优化：`app.exe` 与 `FlipClock.scr` 共享同一 WebView2 数据目录（`%LOCALAPPDATA%\com.flipclock.app\webview-data`），屏保冷启动速度与帧率与手动开启一致。

> 若安装在 Program Files 等受保护目录，注册时可能遇到写权限问题，应用会给出对应提示；必要时以管理员身份运行或手动将 `FlipClock.scr` 复制到 `C:\Windows\System32`。
